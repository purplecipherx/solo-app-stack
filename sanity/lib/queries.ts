export const postsQuery = `*[_type == "post" && !noindex] | order(publishedAt desc) {
  title, "slug": slug.current, seoTitle, metaDescription, excerpt, featuredImage, featuredImageAlt, "category": category->title,
...
export const toolsQuery = `*[_type == "tool"] | order(name asc) {
  name, "slug": slug.current, logo, logoUrl, image, imageAlt, shortDescription, bestFor, pricingSummary,
